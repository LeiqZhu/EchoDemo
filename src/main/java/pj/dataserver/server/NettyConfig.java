...
import pj.dataserver.conf.imp.NutJsonConfig;
...
public class NettyConfig {
    ...
    public NettyConfig(String fileName) {
        ...
        try {
            if (fileName.endsWith(".js") || fileName.endsWith(".json")) {
                NutJsonConfig config = new NutJsonConfig(fileName, "default-nett");
                initConfig(config);
     ...
     public NettyConfig initProperties(Configuration pc) {
         ...
         this.default404Info = pc.getString("nett.404info", this.default404Info);
    ...
    public NettyConfig initConfig(NutJsonConfig pc) {
        ...
        this.ioThreadCoreNum = pc.getInt("nett.ioThreadCoreNum", this.ioThreadCoreNum);
        this.ioThreadMaxNum = pc.getInt("nett.ioThreadMaxNum", this.ioThreadMaxNum);
        this.maxWorkerInQueue = pc.getInt("nett.maxWorkerInQueue", this.maxWorkerInQueue);
        ...
        this.blockTimeout = pc.getInt("nett.blockTimeout", this.blockTimeout);

        this.poolLogSpan = pc.getInt("nett.poolLogSpan", this.poolLogSpan);

        this.useReqObjectPool = pc.getBoolean("nett.useObjectPool", this.useReqObjectPool);
        this.initCacheSpanTime = pc.getInt("nett.initCacheSpanTime", this.initCacheSpanTime);
        ...
        this.default404Info = pc.getString("nett.404info", this.default404Info);
         ...
         this.isStartPaySchedule = pc.getBoolean("nett.is_start_pay_schedule",
                                                this.isStartPaySchedule);

         this.isUseLocalCache = pc.getBoolean("netty.is_use_local_cache", this.isUseLocalCache);

         this.isUseLocalDataCache = pc.getBoolean("nett.is_use_local_data_cache",
